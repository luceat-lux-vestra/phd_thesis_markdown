...
\setmainhangulfont[Ligatures=Common,
                   WordSpace=0.8,
                   LetterSpace=1.0]{NanumMyeongjo}
...
\setmainfont[Ligatures=Common,
             WordSpace=0.6,
             LetterSpace=1.0]{NanumMyeongjo}
\setsansfont[Ligatures=Common,
             WordSpace=1.0,
             LetterSpace=1.0]{NanumGothic}
\setmonofont{Fira Code}
\renewcommand{\familydefault}{\sfdefault}
% https://tex.stackexchange.com/questions/49831/change-body-font-in-moderncv-without-affecting-the-other-fonts
% https://tex.stackexchange.com/questions/324030/simple-way-to-format-section-titles
...
\lstdefinestyle{javaCodeStyle}{
  ...
  basicstyle=\fontsize{10}{8}\ttfamily,
...
위에 저 소나무 철갑을 두른 듯 바람서리 불변함은 우리 기상일세 가을 하늘
공활한데 높고 구름 없이 밝은 달은 우리 가슴 일편단심일세 이 기상과 이
맘으로 충성을 다하여 괴로우나 즐거우나 나라 사랑하세 무궁화 삼천리 화려
강산 대한 사람 대한으로 길이 보전하세\footnote{test}
...
\KeywordTok{public} \KeywordTok{class}\NormalTok{ Main }\OperatorTok{\{}
  ...
  \KeywordTok{public} \DataTypeTok{static} \DataTypeTok{void} \FunctionTok{main}\OperatorTok{(}\BuiltInTok{String}\OperatorTok{[]}\NormalTok{ args}\OperatorTok{)} \OperatorTok{\{}
    ...
    \DataTypeTok{int}\NormalTok{ num }\OperatorTok{=} \DecValTok{29}\OperatorTok{;}
    \DataTypeTok{boolean}\NormalTok{ flag }\OperatorTok{=} \KeywordTok{false}\OperatorTok{;}
    \ControlFlowTok{for} \OperatorTok{(}\DataTypeTok{int}\NormalTok{ i }\OperatorTok{=} \DecValTok{2}\OperatorTok{;}\NormalTok{ i }\OperatorTok{\textless{}=}\NormalTok{ num }\OperatorTok{/} \DecValTok{2}\OperatorTok{;} \OperatorTok{++}\NormalTok{i}\OperatorTok{)} \OperatorTok{\{}
      \CommentTok{// 한글 condition for nonprime number}
      \ControlFlowTok{if} \OperatorTok{(}\NormalTok{num }\OperatorTok{\%}\NormalTok{ i }\OperatorTok{==} \DecValTok{0}\OperatorTok{)} \OperatorTok{\{}
\NormalTok{        flag }\OperatorTok{=} \KeywordTok{true}\OperatorTok{;}
        \ControlFlowTok{break}\OperatorTok{;}
      \OperatorTok{\}}
    \OperatorTok{\}}

    \ControlFlowTok{if} \OperatorTok{(!}\NormalTok{flag}\OperatorTok{)}
      \BuiltInTok{System}\OperatorTok{.}\FunctionTok{out}\OperatorTok{.}\FunctionTok{println}\OperatorTok{(}\NormalTok{num }\OperatorTok{+} \StringTok{" is a prime number."}\OperatorTok{);}
    \ControlFlowTok{else}
      \BuiltInTok{System}\OperatorTok{.}\FunctionTok{out}\OperatorTok{.}\FunctionTok{println}\OperatorTok{(}\NormalTok{num }\OperatorTok{+} \StringTok{" is not a prime number."}\OperatorTok{);}
  \OperatorTok{\}}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
